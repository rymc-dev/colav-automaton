...
@pytest.mark.parametrize(  
    "waypoints",
     ...
     ids=[
        "Test Case 1: No waypoints in the auxiliary state, pop should raise exception",
        "one waypoint in the auxiliary state, pop should raise exception", 
        "multiple waypoints in the auxiliary state, pop should remove the most significant waypoint from the latest state"
     ]
)
def test_pop_waypoint(waypoints): 
    ... 
